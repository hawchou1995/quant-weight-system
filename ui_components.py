...
function mktBadgeHtml(){
  ...
  /* 盘中实时（R-live-0918）：行情源时间戳证明今天在交易 → 覆盖构建期 tradeDay
     （否则上一交易日构建的页面在新交易日会一直显示"休市"） */
  var lv=window.INTRADAY&&window.INTRADAY.state;
  if(lv&&lv.live){s={isTradingDay:true,tradeDay:today};}
  ...
  return '<span class="mkt-badge '+cls+'" title="交易日 '+((s.tradeDay)||'—')+' · '+((lv&&lv.live)?'实时探测(行情源时间戳)':'本地时钟判定')+'"><span class="d"></span>'+txt
