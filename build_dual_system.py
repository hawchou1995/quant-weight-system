...
def bt_short_html():
    """短线回测参考 5 卡（股票分层 4 + 基金；2026-08-17 去 ETF）"""
    cards = "".join([
...
<h2>📌 全量池短线跟踪 <span class="badge badge-auto">自动 · 保留 30 天</span> <span class="count" id="watch-count" style="font-size:12px"></span></h2>
...
document.addEventListener('DOMContentLoaded',function(){{
  ...
  function renderWatch(){{
    ...
    Object.keys(track).forEach(function(code){{
      ...
      rows.push({{code:code,entry:t.entry||'—',age:age,type:t.type||''}});
    }});
    // 按入池日期倒序（新入池在前，2026-08-17 可读性优化）
