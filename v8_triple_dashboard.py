...
auto_extra = [
    ...
    {"type": "text", "tab": "auto", "title": "分档配置表（资金 × 权限 × TopN，数据说话）",
     "text": "- **权限档位**（引擎 perm 参数）：main=仅主板(新开户) / gem=+创业板(2年+10万) / star=+科创板(2年+50万)\n"
             "- **主板(新开户) 推荐 T10**：年化25.1% / 回撤-6.7% / **夏普2.49（全场最高！）** / 57笔每年\n"
             "- **主板+创业板 推荐 T10**：年化22.6% / 回撤-7.9% / 夏普2.44；低频选T3（夏普1.81/+998%/17笔每年）\n"
             "- **全A(含科创) 推荐 T10**：年化22.6% / 回撤-7.9% / 夏普2.34——科创板当前参数下边际为负（高波动尾部）\n"
             "- **资金档**：自动补位让资金几乎无差异（10万vs300万<1%），任何资金可跑任何档\n"
             "- **关键发现**：无权限不是劣势——主板池去掉创业板/科创高回撤尾部（2018-2022阴跌），核心收益（2024-2026主板AI标的）保留\n"
             "- **选择逻辑**：先看自己的权限档 → 再按频率偏好选 TopN（T3低频17笔/年 ~ T10高频57笔/年）→ 资金任意"},
