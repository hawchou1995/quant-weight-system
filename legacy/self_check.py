...
check(f"关注标的 {len(data_files)} 只（期望 24）", len(data_files) == 24, f"实际 {len(data_files)}")
...
if sum_path.exists():
    try:
        ...
        check("组合总收益 ≈ 186.64%", ret is not None and abs(ret - 186.64) < 1.0, f"实际 {ret}%")
...
if snap_path.exists():
    try:
        ...
        sigs = d.get("signals", [])
        check(f"快照 24 标的", len(sigs) == 24, f"实际 {len(sigs)}")
