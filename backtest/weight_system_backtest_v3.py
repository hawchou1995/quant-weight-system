# -*- coding: utf-8 -*-
"""
综合指标权重系统回测引擎 v3（2026-08-12 · 轻量动态版）
================================================================
在 v2（27 池 × 9 领域，2024-01 起）基础上，按用户 grill 决策新增五项增量，
每项独立开关可回退（可逆优先）：

1. USE_MA_TRIPLE  均线 5/20/60 三线状态分（知识库：5日强弱/20日方向/60日大势）
                   → 并入趋势类（MA20位置30/ADX25/mom20 20/三线25）
2. USE_BOLL_POS   布林带 %B 位置分（口诀"布林看位置"）
                   → 并入超买超卖类（RSI45/KDJ30/布林25）
3. USE_RESO_PATCH 三重指标共振补丁（大财师兄：KDJ情绪+MACD趋势+均线方向）
                   n_bull=3 → trend/momentum 各 +10；=2 → +5；≤1 → -10
4. USE_BULL_BEAR  轻牛熊系数（只乘进攻类 trend/momentum/volume，风控恒 1.0）：
                   沪深300 MA20 偏离连续映射 0.95~1.05（dev∈±5% 截断）
                   × 年线两档 0.92/1.08（close vs MA250）
5. WT_*           三权重候选：甲 30/25/15/15/10/5 ｜ 乙 32/26/14/13/10/5 ｜ 丙 33/25/15/13/8/6

验收标准（27 池等权组合）：收益 ≥ v2 基线 +130.75% 且最大回撤 ≤ 20%。
运行：python weight_system_backtest_v3.py
输出：weight_system_v3_results.json（多实验对比）
"""
import pandas as pd
import numpy as np
import json
...
USE_MA_TRIPLE = True        # v3-1 均线 5/20/60 三线状态分
USE_BOLL_POS = True         # v3-2 布林带 %B 位置分
USE_RESO_PATCH = True       # v3-3 三重指标共振补丁
USE_BULL_BEAR = True        # v3-4 轻牛熊系数（进攻类乘法）
...
# 5. 回测引擎（v2 + 牛熊系数传入）
# ---------------------------------------------------------------------------
def run_backtest(df, news_level, market_state=None, bb_coef=None, weights=None, strategy="weight"):
...
# 7. 主流程（多实验对比）
# ---------------------------------------------------------------------------
def run_experiment(name, weights, switches, market_state, bb_coef):
    ...
    for sector, members in SECTOR_POOL.items():
        for code, nm, typ, market, news_level in members:
            df = load_data(code)
            eq_w, tr_w, turo = run_backtest(df, news_level, market_state, bb_coef, weights, strategy="weight")
            eq_b, tr_b, _ = run_backtest(df, news_level, market_state, bb_coef, weights, strategy="bh")
    ...
    # 恢复开关
    USE_MA_TRIPLE, USE_BOLL_POS, USE_RESO_PATCH, USE_BULL_BEAR = old
    return {
...
def main():
    print(f"v3 回测：{BACKTEST_START} ~ {BACKTEST_END}，27 标的 × 9 领域")
    ...
    experiments = [
        # 基线复现（v3 全部增量关，权重甲）——验证引擎与 v2 一致
        ("基线v2复现(全关/甲)", WT_A, {"ma_triple": False, "boll_pos": False, "reso_patch": False, "bull_bear": False}),
        # v3 全开 × 三权重
        ("v3全开/甲", WT_A, {"ma_triple": True, "boll_pos": True, "reso_patch": True, "bull_bear": True}),
        ("v3全开/乙", WT_B, {"ma_triple": True, "boll_pos": True, "reso_patch": True, "bull_bear": True}),
        ("v3全开/丙", WT_C, {"ma_triple": True, "boll_pos": True, "reso_patch": True, "bull_bear": True}),
        # 单项归因（权重甲，单开一项）
        ("仅牛熊系数", WT_A, {"ma_triple": False, "boll_pos": False, "reso_patch": False, "bull_bear": True}),
        ("仅共振补丁", WT_A, {"ma_triple": False, "boll_pos": False, "reso_patch": True, "bull_bear": False}),
        ("仅均线三线", WT_A, {"ma_triple": True, "boll_pos": False, "reso_patch": False, "bull_bear": False}),
        ("仅布林位置", WT_A, {"ma_triple": False, "boll_pos": True, "reso_patch": False, "bull_bear": False}),
        # 排除组合（牛熊+共振，权重甲）
        ("牛熊+共振", WT_A, {"ma_triple": False, "boll_pos": False, "reso_patch": True, "bull_bear": True}),
    ]

    out_exps = []
    for name, wt, sw in experiments:
        print(f"\n=== 实验：{name} ===")
        exp = run_experiment(name, wt, sw, market_state, bb_coef)
    ...
    out = {
        ...
        "pool": "27标的×9领域等权组合",
        "baseline_note": "v2 基线：+130.75% / 回撤 14.64% / 夏普 1.643 / 胜率 84.7%",
        "acceptance": "收益 ≥ +130.75% 且最大回撤 ≤ 20%",
        "experiments": out_exps,
    }
    with open("weight_system_v3_results.json", "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print("\n已输出 weight_system_v3_results.json")
