...
import rev_screen  # R-daban-negscreen-0919 反向筛闸门
...
_ALT_DATA = r"D:/Documents/Quant data/data_full"
DATA_DIR = _ALT_DATA if os.path.isdir(_ALT_DATA) else os.path.join(os.path.dirname(os.path.dirname(BASE)), "data_full")  # 云端回落仓库 data_full
NAMES_JSON = os.path.join(os.path.dirname(os.path.dirname(BASE)), "data_full_names.json")
IND_JSON = os.path.join(os.path.dirname(os.path.dirname(BASE)), "stock_industry.json")
...
def main():
    ...
    _rs = rev_screen.stats(D)
    if _rs.get("covered"):
        print(f'  反向筛闸门: 当日回避 %d 只 / 宇宙 %d 只（只拦新开仓）'
              % (_rs["avoided_symbols"], _rs["universe"]))
    elif _rs.get("enabled"):
        print(f'  !! 反向筛闸门告警: {D} 不在资产覆盖内（资产至 {_rs.get("asset_last")}，'
              f'缺口 {_rs.get("gap_days")} 天）→ 本轮闸门失效，需重生成资产')
    ...
    for w in state.get('watchlist', []):
        ...
        if feat['is_yz'][i]:
            ...
            continue
        if rev_screen.hit(code, D):
            print(f'  [反向筛剔除] {code} {w["name"]} {D} 命中 9 强臂回避集，不入场')
            continue
    ...
    # 2026-09-18 修复：幂等早退原先在阶段2 之前 → 阶段1（写观察清单）被一并跳过，
    # 导致「当天数据在中途补齐后重扫」时新发现的首板永远进不了清单（实测：全景 3 命中 vs
    # 清单 1 只）。现移到阶段1 之后：阶段2 天然幂等（已入场条目已从清单移除，重跑找不到），
    # 阶段1 自身按 (code, sb_date) 去重，故重扫安全。
    if state.get('last_scan') == D:
        print(f'  已处理过 {D}：仅刷新观察清单与全景（阶段2 幂等跳过）')
        save_state(state)
