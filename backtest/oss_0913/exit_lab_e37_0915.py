# -*- coding: utf-8 -*-
"""R-macd-kdj-event-0915 · E3 修正案 + E7 轮2过门臂 组合级（轨B 载体）：BASE / pct40 / e3 / e7 / pct40+e3 / pct40+e7"""
...
# ---- E3/E7 顶背离（R-macd-kdj-event-0915：E3=首轮边缘修正案，E7=轮2 过门臂）：20日新高且 DIF 不新高；E7 叠加 J>80 ----
    ...
    for di in range(WARMUP, ND):
        ...
        if exit_mode:
            for j in np.flatnonzero(shares > 0):
                ...
                elif exit_mode == "e3":    trig = bool(E3[di, jj])   # 顶背离
...
res = {}
for mode in [None, "pct40", "e3", "e7", "pct40e3", "pct40e7"]:
    shs, anns = [], []
    for off in range(F):
...
# ---- 结果落盘 ----
json.dump({k: v for k, v in res.items()}, open(OUT / 'exit_lab_e37_0915.json', 'w', encoding='utf-8'),
          ensure_ascii=False, indent=1, default=float)
log('saved exit_lab_e37_0915.json')
