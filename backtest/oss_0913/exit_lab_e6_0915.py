# -*- coding: utf-8 -*-
"""R-macd-kdj-event-0915 · E6 单阴背离组合级（轨B 载体）：BASE / pct40 / e6 / pct40+e6"""
...
# ---- E6 单阴背离（R-macd-kdj-event-0915 过门臂）：当日跌幅 ≤−6% 且 MACD 柱 >0 → T+1 开盘出场 ----
_SB = pd.DataFrame(close_ff)
_e12b = _SB.ewm(span=12, adjust=False).mean(); _e26b = _SB.ewm(span=26, adjust=False).mean()
_difb = _e12b - _e26b; _deab = _difb.ewm(span=9, adjust=False).mean()
HISTB = (_difb - _deab).to_numpy()
CHG1B = _SB.pct_change().to_numpy()
E6 = (CHG1B <= -0.06) & (HISTB > 0)
log("轨B matrices ok")
    ...
    for di in range(WARMUP, ND):
        ...
        if exit_mode:
            for j in np.flatnonzero(shares > 0):
                ...
                elif exit_mode == "e6":    trig = bool(E6[di, jj])   # 单阴背离
...
res = {}
for mode in [None, "pct40", "e6", "pct40e6"]:
    shs, anns = [], []
    for off in range(F):
...
# ---- 结果落盘 ----
json.dump({k: v for k, v in res.items()}, open(OUT / 'exit_lab_e6_0915.json', 'w', encoding='utf-8'),
          ensure_ascii=False, indent=1, default=float)
log('saved exit_lab_e6_0915.json')
