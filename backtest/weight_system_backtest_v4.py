# -*- coding: utf-8 -*-
"""
综合指标权重系统回测引擎 v4（2026-08-12 · 量价强化候选）
================================================================
基线 = v3 选型（权重甲 + 布林位置分；共振补丁/牛熊系数/均线三线默认关）。
v4 新增量价候选（grill 共识，2026-08-12）：
1. USE_EXTREME_VOL  地量见底/天量见顶（双确认：量比<0.6 且 20日量新低 = 地量；
                    量比>2.5 且 20日量新高 = 天量；位置门控：低位地量加分、高位天量减分）
2. USE_PDV          纯量价背离（价创20日新高但量未同步新高=顶背离减分；
                    价创20日新低但量未同步新低=底背离加分）→ volume 类
3. USE_RSI_DIV      RSI 背离 + 量能验证（价新高但RSI高点变矮=顶背离；价新低但RSI低点抬高=底背离；
                    顶背离缩量更可信、底背离放量更靠谱）→ osc 类
SCORE_MODE: "patch"=补丁式±10分 / "evidence"=分支内证据式（cap/floor）
验收标准（27 池等权组合）：收益 ≥ v3 基线 +138.63% 且最大回撤 ≤ 20%；
补丁式若换手率翻倍（>1.7%）即判负（v3 共振补丁教训）。
运行：python weight_system_backtest_v4.py
输出：weight_system_v4_results.json（多实验对比）
"""
import pandas as pd
import numpy as np
import json
...
def run_experiment(name, weights, switches, market_state, bb_coef):
    ...
    (USE_MA_TRIPLE, USE_BOLL_POS, USE_RESO_PATCH, USE_BULL_BEAR,
     USE_EXTREME_VOL, USE_PDV, USE_RSI_DIV, SCORE_MODE) = old
    return {
...
def main():
    print(f"v4 量价候选回测：{BACKTEST_START} ~ {BACKTEST_END}，27 标的 × 9 领域")
    ...
    experiments = [
        # 基线（v3 选型：权重甲 + 布林位置分）
        ("基线v3选型", WT_A, dict(BASE)),
        # v4-1 地量见底/天量见顶（证据式）
        ("仅地量天量(ev)", WT_A, {**BASE, "extreme_vol": True, "score_mode": "evidence"}),
        # v4-2 纯量价背离（补丁式 vs 证据式）
        ("仅量价背离(patch)", WT_A, {**BASE, "pdv": True, "score_mode": "patch"}),
        ("仅量价背离(ev)", WT_A, {**BASE, "pdv": True, "score_mode": "evidence"}),
        # v4-3 RSI 背离 + 量能验证（补丁式 vs 证据式）
        ("仅RSI背离(patch)", WT_A, {**BASE, "rsi_div": True, "score_mode": "patch"}),
        ("仅RSI背离(ev)", WT_A, {**BASE, "rsi_div": True, "score_mode": "evidence"}),
        # 组合（全证据 / 全补丁 / 量能类组合）
        ("量价三件套(ev)", WT_A, {**BASE, "extreme_vol": True, "pdv": True, "rsi_div": True, "score_mode": "evidence"}),
        ("量价三件套(patch)", WT_A, {**BASE, "extreme_vol": True, "pdv": True, "rsi_div": True, "score_mode": "patch"}),
        ("地量天量+量价背离(ev)", WT_A, {**BASE, "extreme_vol": True, "pdv": True, "score_mode": "evidence"}),
    ]
    ...
    out = {
        ...
        "pool": "27标的×9领域等权组合",
        "baseline_note": "v3 选型基线：+138.63% / 回撤 13.91% / 胜率 89.4% / 180 笔 / 换手 0.845%",
        "acceptance": "收益 ≥ +138.63% 且最大回撤 ≤ 20%；补丁式换手 >1.7% 判负（v3 共振补丁教训）",
        "experiments": out_exps,
    }
    with open("weight_system_v4_results.json", "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print("\n已输出 weight_system_v4_results.json")
