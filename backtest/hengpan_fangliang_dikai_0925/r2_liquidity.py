# -*- coding: utf-8 -*-
"""r2: tradability / artifact hunt — does the ranking edge survive a higher liquidity floor?"""
import json, pathlib, numpy as np, pandas as pd
S = pathlib.Path(r"C:/Users/Admin/.pi-desktop/scratch/d40abb6e-dc12-4d27-8ed2-f290de0a6c5c")
R = pathlib.Path(r"D:/Documents/Workbuddy/股票基金/quant-weight-system")
U = json.loads((R/"backtest/wechat_hotspot_leader_0925/universe.json").read_text(encoding="utf-8"))
cal = np.array(U["calendar"], dtype=object); T, N = len(cal), len(U["universe"])
O = np.asarray(np.load(S/"wl_OPEN.npy", mmap_mode="r")); C = np.asarray(np.load(S/"wl_CLOSE.npy", mmap_mode="r"))
AMT20 = np.asarray(np.load(S/"wl_AMT20.npy", mmap_mode="r")); NV = np.asarray(np.load(S/"wl_NV.npy", mmap_mode="r"))
ATR20 = np.asarray(np.load(S/"wl_ATR20.npy", mmap_mode="r")); RET20 = np.asarray(np.load(S/"wl_RET20.npy", mmap_mode="r"))
VOLBR = np.asarray(np.load(S/"wl2_VOLBR.npy", mmap_mode="r")); GAPO = np.asarray(np.load(S/"wl2_GAPNEXT.npy", mmap_mode="r"))
VALID = np.load(S/"wl_VALID.npy").astype(bool)
IDX = pd.read_csv(R/"index_000300.csv"); IDX["date"]=IDX["date"].astype(str).str.slice(0,10)
BC = pd.to_numeric(IDX.set_index("date").reindex(cal)["close"], errors="coerce").to_numpy(float)
def sh(a,k):
    b=np.full(a.shape,np.nan,dtype=np.float32); b[:-k]=a[k:]; return b
E1=sh(O,1); C2=sh(C,2); C1=sh(C,1); RT=C2/E1-1.0
RT[~(np.isfinite(C2)&(C2>0)&np.isfinite(E1)&(E1>0))]=np.nan
rng = np.random.default_rng(20260925)
def make_pool(MINAMT, MINPX=2.0):
    okb=np.zeros((T,N),bool); POOL={}
    for t in range(60,T-3):
        okb[t]=(VALID[t]&(NV[t]>=250)&(C[t]>=MINPX)&np.isfinite(AMT20[t])&(AMT20[t]>=MINAMT))
        g=GAPO[t]
        m=okb[t]&np.isfinite(g)&(g<=-0.01)&(g>=-0.03)&np.isfinite(RT[t])
        idx=np.nonzero(m)[0]
        if idx.size: POOL[t]=idx
    return okb, POOL
def tstat(x): return float(x.mean()/(x.std(ddof=1)/np.sqrt(x.size))) if x.size>1 and x.std(ddof=1)>0 else 0.0
def sc_of(key,t,idx):
    if key=="amt_lo":   s=-AMT20[t][idx]
    elif key=="volbr_lo": s=-VOLBR[t][idx]
    elif key=="ret20_lo": s=-RET20[t][idx]
    elif key=="gap_deep": s=-GAPO[t][idx]
    elif key=="random":   s=rng.random(idx.size)
    return np.where(np.isfinite(s), s, -1e30)
def sel(key,K,POOL):
    o={}
    for t,idx in POOL.items():
        o[t]= idx if len(idx)<=K else idx[np.argsort(-sc_of(key,t,idx))][:K]
    return o
def ev(sig,POOL):
    ev_=[]; sdm=[]; poolm=[]
    for t,idx in sig.items():
        v=RT[t][idx]; v=v[np.isfinite(v)]
        if not v.size: continue
        pv=RT[t][POOL[t]]; pv=pv[np.isfinite(pv)]
        ev_.append(v); sdm.append(v.mean()*100); poolm.append(pv.mean()*100)
    a=np.concatenate(ev_)*100
    return dict(n=int(a.size), mean=round(float(a.mean()),4), med=round(float(np.median(a)),4),
                wr=round(float(100*(a>0).mean()),2), t_sig=round(tstat(np.array(sdm)),2),
                t_exc=round(tstat(np.array(sdm)-np.array(poolm)),2))
def sim(sig,KSLOT,cost_side=0.0010):
    ts=min(sig); cash=1.0; pos={}; nav=np.full(T,np.nan); trades=[]; nav[ts]=1.0
    for t in range(ts+1,T):
        for _ in range(2):
            for j in [j for j,p in pos.items() if p["tgt"]<=t]:
                p=pos[j]; px=C[t,j]
                if np.isfinite(px) and px>0:
                    pos.pop(j); cash+=p["sh"]*px*(1-cost_side)
                    trades.append((str(cal[t])[:4],(px*(1-cost_side)/(p["px"]*(1+cost_side))-1)*100))
                else: p["tgt"]=t+1
        for j in sig.get(t-1,[]):
            if len(pos)>=KSLOT: continue
            if j in pos: continue
            px=O[t,j]
            if not (np.isfinite(px) and px>0): continue
            navprev=nav[t-1] if np.isfinite(nav[t-1]) else 1.0
            alloc=min(navprev/KSLOT,cash)
            if alloc<=1e-12: break
            pos[j]=dict(en=t,tgt=t+2,sg=t-1,sh=alloc/(px*(1+cost_side)),px=px); cash-=alloc
        mv=0.0
        for j,p in pos.items():
            c=C[t,j]; mv+=p["sh"]*(c if np.isfinite(c) and c>0 else p["px"])
        nav[t]=cash+mv
    bnav=np.full(T,np.nan); bnav[ts]=1.0
    for t in range(ts+1,T): bnav[t]=BC[t]/BC[ts]
    ok=np.isfinite(nav); v=nav[ok]; b=bnav[ok]; nd=len(v)-1
    ann=(v[-1]/v[0])**(244/nd)-1; pk=np.maximum.accumulate(v); dr=v[1:]/v[:-1]-1; sd=dr.std(ddof=1)
    bann=(b[-1]/b[0])**(244/nd)-1
    r=np.array([x[1] for x in trades],float); ys=sorted(set(x[0] for x in trades))
    return dict(ann=round(ann*100,2), mdd=round(float((v/pk-1).min())*100,2), sharpe=round(float(dr.mean()/sd*np.sqrt(244)),2) if sd>0 else None,
        exc=round((ann-bann)*100,2), n=int(r.size), mean=round(float(r.mean()),3),
        negY=sum(1 for y in ys if np.mean([z[1] for z in trades if z[0]==y])<0), nY=len(ys),
        per_year={y: round(float(np.mean([z[1] for z in trades if z[0]==y])),3) for y in ys})
res={}
print("=== 流动性下限阶梯: 抬下限能否保住排序键的边? (低开[-3,-1], K=10/日, KSLOT=20, 20bp) ===")
print("  %-12s %-10s %7s %8s %9s %9s %7s %7s %8s %8s %7s %7s" % ("下限","排序键","池/日","笔数","毛均%","毛中%","毛胜%","t超额","年化","MDD","夏普","超额pp"))
for MINAMT, lab in ((2e7,"2000万"),(5e7,"5000万"),(1e8,"1亿"),(3e8,"3亿")):
    okb,POOL=make_pool(MINAMT)
    perday=int(np.median([len(v) for v in POOL.values()]))
    for key in ("amt_lo","volbr_lo","ret20_lo","random"):
        sig=sel(key,10,POOL); e=ev(sig,POOL); m=sim(sig,20)
        res["%s|%s"%(lab,key)]=dict(ev=e,port=m,pool_per_day=perday)
        print("  %-12s %-10s %7d %8d %+9.4f %+9.4f %7.2f %7.2f %+8.2f %+8.2f %7.2f %+7.2f" %
              (lab,key,perday,e["n"],e["mean"],e["med"],e["wr"],e["t_exc"],m["ann"],m["mdd"],m["sharpe"] or 0,m["exc"]), flush=True)
# 选中标的的特征（base 池）
okb,POOL=make_pool(2e7)
print("\n=== 选中标的特征 (base 池 2000万, K=10) ===")
for key in ("amt_lo","volbr_lo","ret20_lo","random"):
    sig=sel(key,10,POOL); amt=[]; px=[]; atr=[]; ld=0; ld2=0; nn=0
    for t,idx in sig.items():
        amt.append(AMT20[t][idx]); px.append(C[t][idx]); atr.append(ATR20[t][idx])
        c1=C1[t][idx]; c2=C2[t][idx]
        ok=(np.isfinite(c1))&(c1>0)&(np.isfinite(c2))&(c2>0)
        ld2+=int((c2[ok]/c1[ok]-1<=-0.095).sum()); nn+=int(ok.sum())
        g=GAPO[t][idx]; ld+=int((np.isfinite(g)&(g<=-0.095)).sum())
    amt=np.concatenate(amt); px=np.concatenate(px); atr=np.concatenate(atr)
    print("  %-10s 成交额中位 %7.0f万元 (p10 %6.0f) | 价格中位 %5.2f元 | ATR20中位 %4.2f%% | 入场跌停 %.2f%% | 出场日跌停 %.3f%%" %
          (key, np.median(amt)/1e4, np.percentile(amt,10)/1e4, np.median(px), np.median(atr)*100, 100*ld/len(amt), 100*ld2/max(1,nn)))
print("\n=== amt_lo 逐年 (base 池, 20bp) ===")
e_amt=res["2000万|amt_lo"]["port"]; print("  " + " ".join("%s%+.2f" % (y,v) for y,v in e_amt["per_year"].items()))
print("=== volbr_lo 逐年 ===")
e_v=res["2000万|volbr_lo"]["port"]; print("  " + " ".join("%s%+.2f" % (y,v) for y,v in e_v["per_year"].items()))
print("=== ret20_lo 逐年 ===")
e_r=res["2000万|ret20_lo"]["port"]; print("  " + " ".join("%s%+.2f" % (y,v) for y,v in e_r["per_year"].items()))
json.dump(res, open(S/"r2_liquidity.json","w",encoding="utf-8"), ensure_ascii=False, indent=1)
print("\nsaved r2_liquidity.json")
