...
def main():
    ...
    # 2026-09-23 二次生产变更（用户拍板 B）：X4(止25/损30/持40) -> E2(固定持 2 日 · **不设止盈止损**)
    # 依据：用户成本档 7bp + MTM 口径下 E2 全面占优（夏普 1.217 vs 0.684、年化 +27.29% vs +15.83%、回撤 -21.88% vs -49.90%）；
    #      配对 Δ年化(E2−X4)=+8.21% CI95[-6.74,+24.85]、train +7.46/val +8.91 双盲同向（判定性不足，属方向性决策）；
    #      E2 出场日涨跌停核验通过（0.39%）；「无止损」已由用户明确接受。
    # TP/SL 取有限极值（+999% / -99%）以**禁用**止盈止损；MAXHOLD=2 → 到期日 = 入场日+1（T+2 收盘卖）。
    # 回滚：本行改回 0.25, -0.30, 40 即回到 X4。证据：backtest/报告-qlch-E2vsX4-用户成本档-20260923.md
    TP_PCT, SL_PCT, MAXHOLD = 9.99, -0.99, 2
