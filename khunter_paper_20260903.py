...
# ===== 配置（新优化配置，待生产切换拍板）=====
RSI_BUY = 35          # 入场：RSI_T < 35
RSI_SELL = 55         # 出场：RSI_T > 55（生产仍 75，模拟盘用 55 验证）
LOW_PRICE = 3.0       # 低价过滤：确认日收盘 ≥ 3 元
N_SLOTS = 5           # 最多 5 仓
...
def scan_today(idx, today):
    """扫描今日 T 信号：入场候选（hit & RSI<35 & bear & ≥3元）与持仓出场（RSI>55）
    宇宙过滤镜像生产 build_short_pool：主板 + 剔ST/退市 + 剔停牌 + 20日均成交额≥3000万"""
    bear_t = bool(idx.loc[idx['date'] == today, 'is_bear'].iloc[0]) if (idx['date'] == today).any() else False
    buys, sells = [], []
    n_stock = 0
    mkt_max = today
    for f in sorted(DATA_DIR.glob("*.csv")):
        ...
        if sig is None:
            continue
        n_stock += 1
        if sig['hit'] and sig['rsi'] is not None and sig['rsi'] < RSI_BUY and bear_t and sig['close'] >= LOW_PRICE:
            buys.append({'code': code, 'rsi': sig['rsi'], 'close': sig['close']})
        if sig['rsi'] is not None and sig['rsi'] > RSI_SELL:
            sells.append(code)
    return buys, sells, bear_t, n_stock
...
def main():
    ...
    log(f"确认日 T = {today.date()}  熊市(MA60) = {bool(idx['is_bear'].iloc[-1])}")
    ...
    for code in st["pending_sells"]:
        ...
        st["trades"].append({"code": code, "side": "sell", "date": str(today.date()),
                             ...
                             "reason": f"rsi>{RSI_SELL:.0f}", "hold_days": pos["hold_days"]})
        log(f"  ✅ 卖出 {code} @ {px:.3f} (ret {ret*100:+.2f}%)")
    ...
    for b in st["pending_buys"]:
        ...
        st["trades"].append({"code": code, "side": "buy", "date": str(today.date()),
                             "px": round(px, 4), "reason": "khunter_hit_rsi35_bear"})
    ...
    # ===== 3) 扫描今日信号（明日执行）=====
    buys, sells, bear_t, n_stock = scan_today(idx, today)
    held_codes = {p["code"] for p in st["positions"]}
    held_sells = [c for c in sells if c in held_codes]
    log(f"扫描 {n_stock} 只主板：入场候选 {len(buys)} 只 / RSI>{RSI_SELL:.0f} {len(sells)} 只（其中持仓 {len(held_sells)} 只）")
    for b in buys:
        log(f"  📈 买入候选 {b['code']} (RSI {b['rsi']:.1f}, 收盘 {b['close']:.2f})")
    for c in held_sells:
        log(f"  📉 卖出候选 {c} (RSI>{RSI_SELL:.0f})")
