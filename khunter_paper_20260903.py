...
配置（回测证据：khunter_phase11_ma250 + phase13 grill 审查）：
  - 入场：KHunter 15 信号命中(T) + RSI_T<35 + 熊市判定(hs300<MA250 T) + 主板 + 收盘价≥3元
  - 出场：RSI_T>59（T 确认 → T+1 开盘执行）
  - 仓位：最多 5 仓 × 每仓 ¥20,000（初始资金 ¥100,000，小仓位）
  - 成本：买 0.575% + 卖 0.575%（与回测一致）
  - 时序：今日 T 收盘确认 → 明日 T+1 开盘执行（生产口径，与回测「T-1 确认→T 执行」对齐）

用法：
  python khunter_paper_20260903.py            # 正常日更（收盘后跑，A 轨 RSI_SELL=59）
