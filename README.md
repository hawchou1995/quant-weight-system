...
| **短线全量池** | 股票反转 Top10（剔ST/退市）+ ETF动量10 + 基金动量10 | 短线分 = 动量30/量价25/通道25/波动20 + **MA5生命线每日止损** + 强势市门控（v5.11.7 滑点稳健参数 T10/H10/S55） | 股票 **+315.7% / 回撤-7.1% / 夏普1.75**（含20bps滑点；理想口径+604%/2.38）｜ ETF +103.3% / 1.00（含20bps）｜ 基金 C类 +243.5% / 0.83（含5bps；A类30bps仅+90.7%须用C类） |
...
- **滑点/冲击成本已建模**（每边 bps；佣金万2.5 + 印花税万5 已含）：
  ...
  - 中长线（sweep_ml_slip.py，换手低影响显著更小）：全量池 v9 20/30bps → -23%/-31%（夏普 1.58→1.43）｜ 固定池 v8 → -13%/-18% ｜ ETF 中长线 → -44%/-62%（年 33 笔基数低，绝对收益仍 +63% @20bps）
