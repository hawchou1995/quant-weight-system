...
for rel in "${FILES[@]}"; do
  ...
  for attempt in 1 2 3 4 5; do
    SHA=$(gh api -X POST "$API/git/blobs" --input - <<EOF 2>/dev/null | python -c "import sys,json;print(json.load(sys.stdin)['sha'])" 2>/dev/null || true
...
EOF
  ...
  if [ -z "$SHA" ]; then echo "  ❌ blob 失败(5次): $rel"; exit 1; fi
